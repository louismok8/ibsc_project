...
def main():
    ...
    print("✓ Step 17 complete: Aggregated lesion statistics\n")

    for lesion_id, stats in results.items():

        hit_rate = stats["hit_probability"]
        expected_positive_cores = np.mean(
            stats["distribution_positive_core_counts"]
        )
        mean_percentage_positive = stats["mean_percentage_positive"]
        print(f"Patient: {patient.id}")
        print(f"Lesion {lesion_id}")
        print(f"  True hit-rate: {hit_rate:.3f}")
        print(f"  Expected # positive cores: {expected_positive_cores:.2f}")
        print(f"  Mean % positive core: {100*mean_percentage_positive:.1f}%")
        print("-" * 50)
